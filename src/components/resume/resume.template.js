...
export default function ResumeTemplate(props) {
  ...
  return (
    <>
      <div className="resume__name text-4xl font-bold mb-4 dark:text-white">{name}</div>
      <div className="resume__title text-2xl font-semibold mb-4 dark:text-white">{title}</div>
      <div className="flex flex-wrap resume__contact separators mb-4 dark:text-white">
        <div className="resume__address">{address}</div>
        <div className="resume__phone">{phone}</div>
        <div className="resume__email">{email}</div>
      </div>
      <div className="heading text-xl font-bold mb-4 dark:text-white">Summary</div>
      <div className="resume__objective mb-4 dark:text-white max-w-3xl font-normal leading-6">{objective}</div>
      <div className="resume__work-history mb-4 dark:text-white">
        <div className="heading text-xl font-bold mb-4 dark:text-white">Work History</div>
        {workHistory &&
          workHistory.map(
            ({ startDate, endDate, title, company, location, content }, i) => {
              return (
                <div key={i} className="md:grid md:grid-cols-4 mb-4 dark:text-white">
                  <div className="md:col-span-1 dark:text-white">
                    {startDate} - {endDate}
                  </div>
                  <div className="md:col-span-3 max-w-3xl dark:text-white">
      ...
      <div className="resume__skills mb-4">
        <div className="heading text-xl font-bold mb-4 dark:text-white">Skills</div>
        {skills &&
